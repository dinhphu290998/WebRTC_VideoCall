...
open class KMLDocument: KMLElement {
    ...
    public convenience init? (string: String, generateMapKitClasses: Bool = true) {
        var element: AEXMLElement?
        do {
    ...
    open class func parse(string: String, generateMapKitClasses: Bool=true, callback: @escaping (KMLDocument) -> Void ) {
        let bgQueue = DispatchQueue.global(qos: .default)
        let mainQueue: DispatchQueue = DispatchQueue.main
        bgQueue.async(execute: {
            if let doc: KMLDocument = KMLDocument(string: string, generateMapKitClasses: generateMapKitClasses) {
                mainQueue.async(execute: {
                    callback(doc)
                })
            }
