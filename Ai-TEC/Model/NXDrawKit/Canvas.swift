...
@objc public protocol CanvasDelegate {
    ...
    @objc optional func canvas(_ canvas: Canvas, didUpdate drawing: Drawing, mergedImage image: UIImage?)
    func brush() -> Brush?
}
...
open class Canvas: UIView, UITableViewDelegate {
    ...
    fileprivate let apath = UIBezierPath()
    ...
    fileprivate func currentDrawing1() -> Drawing {
        return Drawing(stroke: self.mainImage.image, background: self.backgroundImageView.image)
    }
    ...
    fileprivate func didUpdateCanvas() {
        ...
        let current = self.currentDrawing1()
        self.delegate?.canvas?(self, didUpdate: current, mergedImage: mergedImage)
    ...
    func strokeImage(rotate: Double) {
        UIGraphicsBeginImageContextWithOptions(self.frame.size, false, 0)

        if self.brush.isEraser {
            // should draw on screen for being erased
            self.mainImage.image?.draw(in: self.frame)
        }

        for x in -11..<Int(gridWidthMultiple) + 11 {
            let start = CGPoint(x: CGFloat(x) * gridWidth, y: -700)
            let end = CGPoint(x: CGFloat(x) * gridWidth, y: self.bounds.height + 300)
            self.apath.move(to: start)
            self.apath.addLine(to: end)
            
            for y in -11..<Int(gridWidthMultiple) + 11 {
                let startY = CGPoint(x: -700, y: CGFloat(y) * gridWidth)
                let endY = CGPoint(x: self.bounds.width + 300, y: CGFloat(y) * gridWidth)
                self.apath.move(to: startY)
                self.apath.addLine(to: endY)
            }
        }
    ...
    @objc open func update(_ backgroundImage: UIImage?) {
        ...
        self.session.append(self.currentDrawing())
        self.session.append(self.currentDrawing1())
    ...
    @objc open func save() {
        ...
        self.drawing.stroke = self.mainImage.image?.copy() as? UIImage
